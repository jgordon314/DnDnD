	...
	setRows(prevRows => prevRows.map(row => {
	  if (row.ability_id === abilityId) {
		...
		return {
		  ...
		  available_uses: (row.available_uses !== null && delta > 0)
...
<td className="flex space-x-2">
  <button
	style={{
	  ...
	  color: "#fff",
	  border: "none",
	  cursor: (row.available_uses === null || row.available_uses > 0) ? "pointer" : "default",
	  opacity: (row.available_uses === null || row.available_uses > 0) ? 1 : 0,
	  pointerEvents: (row.available_uses === null || row.available_uses > 0) ? "auto" : "none",
	}}
	disabled={isPending === row.aid || (row.available_uses !== null && row.available_uses === 0)}
  ...
  <button
	...
	disabled={isPending === row.aid || row.activation_count === 0}
	onClick={() => updateActivation(row.ability_id, -1)}>
	-
  </button>
  <button
