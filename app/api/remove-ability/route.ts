...
export async function POST(request: Request) {
	try {
		...
		const { characterId, abilityId } = data;
		...
		await db.query("DELETE FROM CharacterAbilities WHERE character_id = ? AND ability_id = ?", [
			characterId,
			abilityId,
		]);
		...
		return NextResponse.json({ error: "Failed to remove ability from character" }, { status: 500 });
	}
}
